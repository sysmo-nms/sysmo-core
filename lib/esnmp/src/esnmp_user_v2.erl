...
agent(TargetName, Conf) ->
    SnmpmConf = [
        {engine_id, Conf#snmp_agent_conf.engine_id},
        {address,   Conf#snmp_agent_conf.ip},
        {port,      Conf#snmp_agent_conf.port},
        {community, Conf#snmp_agent_conf.community},
        {version,   Conf#snmp_agent_conf.version}
    ],
    call({agent, atom_to_list(TargetName), SnmpmConf}).


%% --- Various SNMP operations ----

sync_get(TargetName, Oids, Timeout) ->
    call({sync_get, TargetName, Oids, Timeout}).
...
handle_call({sync_get, TargetName, Oids, Timeout}, _From, S) ->
    Reply = (catch snmpm:sync_get(?USER, TargetName, Oids, Timeout)),
    {reply, Reply, S};
