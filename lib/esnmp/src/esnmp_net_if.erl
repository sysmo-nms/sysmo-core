...
handle_recv_msg(Addr, Port, Bytes, 
    ...
    case (catch snmpm_mpd:process_msg(Bytes, Domain, Addr, Port, MpdState, NoteStore, Logger)) of

	    {ok, Vsn, Pdu, MS, ACM} ->
	        maybe_handle_recv_pdu(Addr, Port, Vsn, Pdu, MS, ACM, Logger, State);

	    {discarded, Reason, Report} ->
	        ?vdebug("discarded: ~p", [Reason]),
	        ErrorInfo = {failed_processing_message, {Reason, Addr, Port}},
	    ...
	    Error ->
	        error_msg("processing of received message failed: ~n   ~p", [Error]),
	        ok
    end.
...
handle_recv_pdu(Addr, Port, Vsn, #pdu{type = 'inform-request'} = Pdu,
                _PduMS, ACM, Logger, #state{server = Pid, irb = IRB} = State) ->
    handle_inform_request(IRB, Pid, Vsn, Pdu, ACM, Addr, Port, Logger, State);

handle_recv_pdu(Addr, Port, _Vsn, #pdu{type = report} = Pdu,
                _PduMS, ok, _Logger, #state{server = Pid} = _State) ->
    ?vtrace("received report - ok", []),
    Pid ! {snmp_report, {ok, Pdu}, Addr, Port};

handle_recv_pdu(Addr, Port, _Vsn, #pdu{type = report} = Pdu, 
                _PduMS, {error, ReqId, Reason}, _Logger, #state{server = Pid} = _State) ->
    ?vtrace("received report - error", []),
    Pid ! {snmp_report, {error, ReqId, Reason, Pdu}, Addr, Port};

handle_recv_pdu(Addr, Port, _Vsn, #pdu{type = 'snmpv2-trap'} = Pdu,
                _PduMS, _ACM, _Logger, #state{server = Pid} = _State) ->
    ?vtrace("received snmpv2-trap", []),
    %io:format("handle_pdu from ~p ~p~n", [?MODULE, ?LINE]),
    Pid ! {snmp_trap, Pdu, Addr, Port};

handle_recv_pdu(Addr, Port, _Vsn, Trap, _PduMS, _ACM, _Logger, 
		        #state{server = Pid} = _State) when is_record(Trap, trappdu) ->
    ?vtrace("received trappdu", []),
    %io:format("handle_pdu from ~p ~p~n", [?MODULE, ?LINE]),
    Pid ! {snmp_trap, Trap, Addr, Port};

handle_recv_pdu(Addr, Port, _Vsn, Pdu, _PduMS, _ACM, 
		        _Logger, #state{server = Pid} = _State) when is_record(Pdu, pdu) ->
    ?vtrace("received pdu", []),
