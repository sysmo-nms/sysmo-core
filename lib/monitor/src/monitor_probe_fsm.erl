...
% local api
-export([
    freeze/1,           % from monitor_probe_sup:parents_sync/0
    reset_family/1,
    synchronize_parents/1,  % from monitor_probe_sup:parents_sync/0
...
emit_local(SData, PR) ->
    ...
    Pdu = monitor_pdu:probe_return({PR, TargetId, Name}),
...
emit_wide(NSData, PR) ->
    ...
    monitor_target_channel:update(TargetId, ProbeId, {Probe, PR}).
